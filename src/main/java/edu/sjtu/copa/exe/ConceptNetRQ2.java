...
public class ConceptNetRQ2 {
	...
	// here, we tune the alpha, beta as the cikm paper described
		...
		static String conceptQ2 = ConfigManager.getConfig("light-copa-config.ini", "input-dir", "CONCEPTNET_RQ2"); //"concept_format3.txt";
		static String logdir = ConfigManager.getConfig("light-copa-config.ini", "output-dir", "LOG_PATH");//"testLog_conceptnetRQ2/";
		...
		public static void apply(String logdir, String logfilename, String filename) throws IOException{
			...
			while((line1 = reader.readLine()) != null &&
				...
				double s = 0.0;
				if (cnt < 518){ // 222个binary choice 算444次分
					s = calScore(processedctree, processedetree, logger);
				}
				else {
					s = calScore(processedctree, processedetree, logger);
				}
		...
		public static double calScore(DepTree causeTree, DepTree effectTree, BufferedWriter logger) 
				throws IOException{
	...
	public static double calModelWeightSingleDirection1(DepTree causeTree, DepTree effectTree, String type) throws IOException{
		...
		public static void main(String[] args) throws IOException{
			...
			for (int i=0; i<=10;i++){
				for (int j=0; j<=10; j++){
					alpha = f(i/10.0);
					beta = f(j/10.0);
					String logfilename = "conceptnetRQ2-ruv"+String.valueOf(alpha)+String.valueOf(beta)+"-log"+""+".txt";
					apply(logdir, logfilename, conceptQ2);
				}
			}
			...
			alpha = 1.0;
			beta = -1.0;
			logfilename = "conceptnetRQ2-ruv"+String.valueOf(alpha)+String.valueOf(beta)+"-log"+""+".txt";
			apply(logdir, logfilename, conceptQ2);
