...
public class CausalityPMI {
	public final static String freqFile = ConfigManager.getConfig("light-copa-config.ini", "input-dir", "CAUSAL_WORD_COPA");
	...
	static HashMap<String,Double> cfreqMap = new HashMap<String,Double>();
	static HashMap<String, Double> efreqMap = new HashMap<String,Double>();
	...
	static {
		...
		try {
			...
			while ((line = reader.readLine()) != null){
				...
				Double cfreq = Double.parseDouble(linePart[1]);
				Double efreq = Double.parseDouble(linePart[2]);
				
				cfreqMap.put(word,cfreq);
				efreqMap.put(word, efreq);
			}
			reader.close();
		} catch (IOException e) {
	...
	public static double calPMI(String x, String y, BufferedWriter writer) throws IOException{
		...
		if (!cfreqMap.containsKey(x) || !efreqMap.containsKey(y)){
		...
		double p_x = cfreqMap.get(x) + efreqMap.get(x);
		double p_y = efreqMap.get(y) + cfreqMap.get(y);
